...
const adminApiGaps = [
  {
    ...
    candidateEndpoints: 'GET/PUT /api/admin/store-appointment-config/stores/{storeCode}',
  ...
  },
  {
    ...
    status: 'missing-admin-api',
    requiredPermission: 'admin:store-appointment-config:manage',
    candidateEndpoints: 'GET/POST /api/admin/store-appointment-config/stores/{storeCode}/projects；PUT /api/admin/store-appointment-config/projects/{projectCode}',
  ...
  },
  {
    ...
    status: 'missing-admin-api',
    requiredPermission: 'admin:store-appointment-config:manage',
    candidateEndpoints: 'GET/POST /api/admin/store-appointment-config/stores/{storeCode}/staff；PUT /api/admin/store-appointment-config/staff/{staffCode}',
  ...
  },
  {
    ...
    status: 'missing-admin-api',
    requiredPermission: 'admin:store-appointment-config:manage',
    candidateEndpoints: 'GET/PUT /api/admin/store-appointment-config/stores/{storeCode}/rules',
...
<template>
  <section>
    ...
    <el-card shadow="never" class="api-gap-panel">
      <template #header>
        <div class="card-header">
          <span>admin API 缺口</span>
          <el-tag type="info" effect="plain">仅规划候选接口</el-tag>
        </div>
      </template>
      <el-alert
        class="readonly-alert"
        type="info"
        title="以下接口尚未实现；它们只是后续受控 backend API 设计清单，当前页面不会创建接口、保存配置或绕过后端写数据。"
        show-icon
      />
      <el-table :data="adminApiGaps" row-key="surfaceKey" size="small">
        ...
        <el-table-column prop="candidateEndpoints" label="候选接口" min-width="360" show-overflow-tooltip />
