...
- 名称：后台 MVP+ 状态同步与用户页空筛选归一化
- OpenSpec 变更：无
- 当前 HEAD：以 Git log 为准

## 当前状态

- 已提交已推送；等待真实后端联调和下一阶段业务页。
...
- 需后续用真实后端数据联调用户页精确筛选、服务请求分页、详情完整手机号、状态更新和后端 403 行为。
...
- GREEN：`npm.cmd run test -- --run src/pages/users/UsersPage.test.ts src/api/adminUsers.test.ts`：2 个测试文件、14 个 Vitest 测试通过。

## 下一步

1. 用真实后端联调用户页精确筛选、服务请求分页、详情完整手机号和状态更新。
