...
- 名称：门店预约 admin 预约详情抽屉组件拆分切片
- OpenSpec 变更：无。承接已完成的门店预约 admin 四配置块 Flow 和组件拆分工作，在 `cekaitech-admin` 内将预约详情抽屉抽成可单独验证的前端组件。
...
- 本轮新增 `StoreAppointmentDetailDrawer.vue`，将预约详情、状态日志和受控状态动作 UI 从 `StoreAppointmentsPage.vue` 中抽出。
- 本轮仅做详情抽屉 UI 组件结构收口，不新增后端接口，不改变详情加载、状态更新和列表刷新逻辑。

## 已完成

- [反馈编号：无] 新增 `src/pages/store-appointments/components/StoreAppointmentDetailDrawer.test.ts`，覆盖详情展示、状态日志数据、受控状态更新事件和无权限隐藏状态动作。
- [反馈编号：无] 新增 `src/pages/store-appointments/components/StoreAppointmentDetailDrawer.vue`。
- [反馈编号：无] `StoreAppointmentsPage.vue` 改为渲染 `<StoreAppointmentDetailDrawer />`，保留父组件内详情加载、状态更新、详情刷新和列表刷新逻辑。
- [反馈编号：无] `docs/变更日志.md` 已记录本轮组件拆分、测试资产和禁区能力边界。

## 最近验证

- RED：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentDetailDrawer.test.ts`
  - 结果：失败于 `StoreAppointmentDetailDrawer.vue` 不存在，符合预期。
- GREEN：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentDetailDrawer.test.ts`
  - 结果：通过，1 个测试文件、2 项。
- 页面回归：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentDetailDrawer.test.ts src/pages/store-appointments/StoreAppointmentsPage.test.ts`
  - 结果：通过，2 个测试文件、30 项。
...
- 收口：`npm.cmd run quality`
  - 结果：通过，56 个测试文件、294 项；`vue-tsc --noEmit` 和 `vite build` 通过，保留既有 PURE 注释和 chunk size warning。

## 未完成

- 尚未本地提交本轮预约详情抽屉组件拆分切片。
- 尚未接入配置审计记录列表入口；当前不能虚构后端审计列表 API。
- 尚未做生产环境发布。

## 下一步

1. 提交 `refactor: extract store appointment detail drawer`，提交正文包含 `Refs: none`。
