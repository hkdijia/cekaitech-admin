...
- 名称：门店预约 admin 项目目录配置 block 切片
- OpenSpec 变更：无。承接 `miniapp-backend` 已验证的门店预约 admin 配置 frontend-ready-contract，在 `cekaitech-admin` 接入项目目录 block 的读取、单项目草稿、保存状态和失败保留输入。
...
- 本地台账：无
- 当前状态：实施中，项目目录 block 定向 TDD 已通过，待完整质量验证和本地提交。
...
- 本轮新增“项目目录配置”block，可读取项目列表、选择单项目编辑草稿并保存。
- 项目目录 block 只保存中性项目字段，`priceText` 仅为展示文案，保存失败时保留用户草稿输入。
- 当前页面仍不开放员工名册、预约规则和回滚编辑入口。
...
- 收口：`npm.cmd run quality`
  - 结果：通过，44 个测试文件、252 项；`vue-tsc --noEmit` 和 `vite build` 通过，保留既有 PURE 注释和 chunk size warning。

## 未完成

- 尚未新增员工名册、预约规则等中性配置面的 admin 编辑入口。
- 门店资料和项目目录 block 尚未抽成独立组件，尚未接入更完整的后端错误码提示表。
- 尚未接入回滚预览、二次确认和执行。
- 尚未做真实支付、会员、核销、客户资料、CRM 跟进、服务记录、员工账号、真实排班、消息通知、退款或客户账户策略。
- 尚未做生产环境发布。
- 尚未本地提交本轮项目目录 block 切片。

## 下一步

1. 本地提交本轮项目目录 block 切片。
2. 下一切片建议按 TDD 接入员工名册 block，仍不触碰员工账号、真实排班、支付、会员、CRM 等能力。
