...
- 名称：门店预约 admin 预约筛选组件拆分切片
- OpenSpec 变更：无。承接已完成的门店预约 admin 四配置块 Flow 和组件拆分工作，在 `cekaitech-admin` 内将预约列表筛选区抽成可单独验证的前端组件。
...
- 本轮新增 `StoreAppointmentFilterPanel.vue`，将预约列表的门店/项目/员工/预约日/状态筛选 UI 从 `StoreAppointmentsPage.vue` 中抽出。
- 本轮仅做筛选 UI 组件结构收口，不新增后端接口，不改变查询参数归一化和分页逻辑。

## 已完成

- [反馈编号：无] 新增 `src/pages/store-appointments/components/StoreAppointmentFilterPanel.test.ts`，覆盖筛选输入、状态选择、search/reset 事件和禁区文案不出现。
- [反馈编号：无] 新增 `src/pages/store-appointments/components/StoreAppointmentFilterPanel.vue`。
- [反馈编号：无] `StoreAppointmentsPage.vue` 改为渲染 `<StoreAppointmentFilterPanel />`，保留父组件内查询参数归一化、分页和 API 调用。
- [反馈编号：无] `docs/变更日志.md` 已记录本轮组件拆分、测试资产和禁区能力边界。

## 最近验证

- RED：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentFilterPanel.test.ts`
  - 结果：失败于 `StoreAppointmentFilterPanel.vue` 不存在，符合预期。
- GREEN：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentFilterPanel.test.ts`
  - 结果：通过，1 个测试文件、1 项。
- 页面回归：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentFilterPanel.test.ts src/pages/store-appointments/StoreAppointmentsPage.test.ts`
  - 结果：通过，2 个测试文件、29 项。
...
- 收口：`npm.cmd run quality`
  - 结果：通过，54 个测试文件、291 项；`vue-tsc --noEmit` 和 `vite build` 通过，保留既有 PURE 注释和 chunk size warning。
...
1. 提交 `refactor: extract store appointment filter panel`，提交正文包含 `Refs: none`。
