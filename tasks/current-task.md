...
- 名称：门店预约 admin 预约查询参数工具收口切片
- OpenSpec 变更：无。承接已完成的门店预约 admin 四配置块 Flow 和组件拆分工作，在 `cekaitech-admin` 内将列表查询参数归一化抽成共享纯工具。
...
- 本轮新增 `storeAppointmentQueryUtils.ts`，统一列表查询参数归一化和 pageSize 上限收敛。
- 本轮仅做查询参数工具结构收口，不新增后端接口，不改变查询字段、分页、详情加载、状态更新或列表刷新逻辑。

## 已完成

- [反馈编号：无] 新增 `src/pages/store-appointments/storeAppointmentQueryUtils.test.ts`，覆盖空白文本归一化、字段 trim、状态保留和 pageSize 最大 100。
- [反馈编号：无] 新增 `src/pages/store-appointments/storeAppointmentQueryUtils.ts`。
- [反馈编号：无] `StoreAppointmentsPage.vue` 改为复用共享查询参数工具，保持 `pageStoreAppointments` 请求字段不变。
...
- RED：`npm.cmd run test -- --run src/pages/store-appointments/storeAppointmentQueryUtils.test.ts`
  - 结果：失败于 `storeAppointmentQueryUtils` 模块不存在，符合预期。
...
- 收口：`npm.cmd run quality`
  - 结果：通过，59 个测试文件、297 项，`vue-tsc --noEmit` 和 `vite build` 通过；保留既有 PURE 注释和 chunk size warning。

## 未完成

- 尚未本地提交本轮预约查询参数工具收口切片。
- 尚未接入配置审计记录列表入口；当前不能虚构后端审计列表 API。
- 尚未做生产环境发布。

## 下一步

1. 提交 `refactor: share store appointment query utils`，提交正文包含 `Refs: none`。
