...
- 名称：门店预约 admin 预约展示工具收口切片
- OpenSpec 变更：无。承接已完成的门店预约 admin 四配置块 Flow 和组件拆分工作，在 `cekaitech-admin` 内将预约状态和时间展示逻辑抽成共享工具。
...
- 本轮新增 `storeAppointmentDisplayUtils.ts`，统一预约状态文案、tag 类型、状态流转文案和时间格式化。
- 本轮仅做展示工具结构收口，不新增后端接口，不改变详情加载、状态更新、列表刷新或分页逻辑。

## 已完成

- [反馈编号：无] 新增 `src/pages/store-appointments/components/storeAppointmentDisplayUtils.test.ts`，覆盖状态文案、tag 类型、未知状态兜底、状态流转和时间格式化。
- [反馈编号：无] 新增 `src/pages/store-appointments/components/storeAppointmentDisplayUtils.ts`。
- [反馈编号：无] `StoreAppointmentListPanel.vue` 和 `StoreAppointmentDetailDrawer.vue` 改为复用共享展示工具，移除重复状态和时间格式化逻辑。
- [反馈编号：无] `docs/变更日志.md` 已记录本轮组件拆分、测试资产和禁区能力边界。

## 最近验证

- RED：`npm.cmd run test -- --run src/pages/store-appointments/components/storeAppointmentDisplayUtils.test.ts`
  - 结果：失败于 `storeAppointmentDisplayUtils` 模块不存在，符合预期。
- GREEN：`npm.cmd run test -- --run src/pages/store-appointments/components/storeAppointmentDisplayUtils.test.ts`
  - 结果：通过，1 个测试文件、1 项。
- 页面回归：`npm.cmd run test -- --run src/pages/store-appointments/components/storeAppointmentDisplayUtils.test.ts src/pages/store-appointments/components/StoreAppointmentListPanel.test.ts src/pages/store-appointments/components/StoreAppointmentDetailDrawer.test.ts src/pages/store-appointments/StoreAppointmentsPage.test.ts`
  - 结果：通过，4 个测试文件、32 项。
...
- 收口：`npm.cmd run quality`
  - 结果：通过，57 个测试文件、295 项，`vue-tsc --noEmit` 和 `vite build` 通过；保留既有 PURE 注释和 chunk size warning。
...
1. 提交 `refactor: share store appointment display utils`，提交正文包含 `Refs: none`。
