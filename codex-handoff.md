...
- 当前分支：`master`
- 当前阶段：门店预约 admin 项目目录配置组件拆分切片。
- 最近完成：新增 `StoreAppointmentServiceCatalogPanel.vue` 和组件测试，`StoreAppointmentsPage.vue` 已改为引用该组件。
- 未完成：本地提交、生产发布。
...
- `npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentServiceCatalogPanel.test.ts`
...
- [反馈编号：无] RED：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentServiceCatalogPanel.test.ts` 失败于 `StoreAppointmentServiceCatalogPanel.vue` 不存在。
- [反馈编号：无] GREEN：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentServiceCatalogPanel.test.ts` 通过，1 个测试文件、4 项。
- [反馈编号：无] 页面回归：`npm.cmd run test -- --run src/pages/store-appointments/StoreAppointmentsPage.test.ts` 通过，1 个测试文件、28 项。
- [反馈编号：无] 定向组合：`npm.cmd run test -- --run src/api/storeAppointments.test.ts scripts/check-admin-integration-ready.test.mjs src/pages/store-appointments/StoreAppointmentsPage.test.ts src/pages/store-appointments/components/StoreAppointmentConfigRollbackPanel.test.ts src/pages/store-appointments/components/StoreAppointmentStoreProfilePanel.test.ts src/pages/store-appointments/components/StoreAppointmentServiceCatalogPanel.test.ts` 通过，6 个测试文件、54 项。
- [反馈编号：无] 收口：`npm.cmd run admin:check` 通过，PASS 13 / WARN 1 / FAIL 0；WARN 为本地 8080 后端未启动。
- [反馈编号：无] 收口：`npm.cmd run quality` 通过，47 个测试文件、275 项，`vue-tsc --noEmit` 和 `vite build` 通过；保留既有 PURE 注释和 chunk size warning。
...
- 本地台账：无
- 当前状态：项目目录组件拆分完整验证已通过，待本地提交。

## 注意事项

- admin 只调用 `miniapp-backend` 受控 API，不直连数据库。
- 项目目录组件只保存中性项目字段：`categoryId/name/summary/durationMinutes/priceText/showPrice/enabled`。
- `priceText` 只是展示文案，不代表真实支付金额、定金、会员权益或核销能力。
...
- 提交信息建议：`refactor: extract store appointment service catalog panel`，正文或 footer 写 `Refs: none`。

## 下一步建议

1. 本地提交本轮项目目录组件拆分切片。
2. 下一切片可继续拆分员工名册或预约规则配置 block。
