...
- 当前分支：`master`
- 当前阶段：门店预约 admin 预约规则配置组件拆分切片。
- 最近完成：新增 `StoreAppointmentRulesPanel.vue` 和组件测试，`StoreAppointmentsPage.vue` 已改为引用该组件。
- 未完成：本地提交、生产发布。
...
- `npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentRulesPanel.test.ts`
- `npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentRulesPanel.test.ts src/pages/store-appointments/StoreAppointmentsPage.test.ts`
...
- [反馈编号：无] RED：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentRulesPanel.test.ts` 失败于 `StoreAppointmentRulesPanel.vue` 不存在。
- [反馈编号：无] GREEN：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentRulesPanel.test.ts` 通过，1 个测试文件、4 项。
- [反馈编号：无] 页面回归：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentRulesPanel.test.ts src/pages/store-appointments/StoreAppointmentsPage.test.ts` 通过，2 个测试文件、32 项。
- [反馈编号：无] 定向组合：`npm.cmd run test -- --run src/api/storeAppointments.test.ts scripts/check-admin-integration-ready.test.mjs src/pages/store-appointments/StoreAppointmentsPage.test.ts src/pages/store-appointments/components/StoreAppointmentConfigRollbackPanel.test.ts src/pages/store-appointments/components/StoreAppointmentStoreProfilePanel.test.ts src/pages/store-appointments/components/StoreAppointmentServiceCatalogPanel.test.ts src/pages/store-appointments/components/StoreAppointmentStaffRosterPanel.test.ts src/pages/store-appointments/components/StoreAppointmentRulesPanel.test.ts` 通过，8 个测试文件、62 项。
- [反馈编号：无] 收口：`npm.cmd run admin:check` 通过，PASS 13 / WARN 1 / FAIL 0；WARN 为本地 8080 后端未启动。
- [反馈编号：无] 收口：`npm.cmd run quality` 通过，49 个测试文件、283 项，`vue-tsc --noEmit` 和 `vite build` 通过；保留既有 PURE 注释和 chunk size warning。
...
- 本地台账：无
- 当前状态：预约规则组件拆分完整验证已通过，待本地提交。

## 注意事项

- admin 只调用 `miniapp-backend` 受控 API，不直连数据库。
- 预约规则组件只保存基础预约规则字段：`bookingWindowDays/defaultDurationMinutes/defaultSlots/confirmationHint/cancelHint`。
- 预约规则不代表真实排班、消息通知、退款规则或客户账户策略。
...
- 提交信息建议：`refactor: extract store appointment rules panel`，正文或 footer 写 `Refs: none`。
