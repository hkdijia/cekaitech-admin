...
- 当前分支：`master`
- 当前阶段：后台 MVP 壳已接入 `miniapp-backend` 开发态登录、工作区、工作区菜单、用户分页/详情、开发态演示数据、用户限制管理和用户主状态调整接口，并具备动态菜单占位页
- 最近完成：Vue 3 工程、后台布局、API 登录、token 本地保存、工作区 API 加载、工作区菜单加载、动态菜单路由、用户管理真实列表页、详情抽屉、演示数据生成、用户状态调整、限制管理页面、测试和构建验证
