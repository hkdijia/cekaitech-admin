...
- 当前分支：`master`
- 当前阶段：admin 小程序工作台法律工具生命周期队列展示和法律工具中心状态流转首片已实现，待最终质量检查、本地提交和用户推送。
- 最近完成：新增 `/miniapp-workbench` 小程序工作台，切换到阳律通工作区后自动进入该工作台；页面调用 backend 工具完整性只读检查接口，按 `enabled/pending_release/blocked/paused/retired` 展示已上线、待发布、阻塞和人工暂缓队列。法律工具中心能力列表新增“状态”下拉，可调用 backend 窄接口人工调整生命周期。
...
- 本地台账：无
- 当前状态：小程序工作台法律工具生命周期队列展示和状态流转首片已实现，待最终质量检查
