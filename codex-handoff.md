...
- 当前分支：`master`
- 当前阶段：门店预约 admin 预约规则配置 block 切片。
- 最近完成：页面新增“预约规则配置”block，支持按 `storeCode` 读取基础预约规则，编辑并保存中性规则字段，保存失败保留草稿输入。
- 未完成：回滚预览/确认/执行、生产发布。
...
- [反馈编号：无] 收口：`npm.cmd run quality` 通过，44 个测试文件、260 项，`vue-tsc --noEmit` 和 `vite build` 通过；保留既有 PURE 注释和 chunk size warning。
...
- 本地台账：无
- 当前状态：预约规则 block 完整质量验证已通过，待本地提交。

## 注意事项

- admin 只调用 `miniapp-backend` 受控 API，不直连数据库。
- 当前页面只开放门店资料、项目目录、员工名册和预约规则中性字段编辑，不开放回滚执行。
...
1. 完成本轮预约规则 block 本地提交。
2. 下一切片进入回滚预览/确认/执行 Flow。
