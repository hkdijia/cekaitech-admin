...
- 当前分支：`master`
- 当前阶段：LMA-FB-018 法律工具来源与交互蓝图管理页，API 客户端、页面、全量质量和 diff 检查已通过。
- 最近完成：在 `/legal-tool-center` 上新增“数据来源”和“交互蓝图”维护标签页，接入后端 V17 新增后台 API。
- 未完成：提交、推送和企业微信回写。
...
1. 提交：`feat: add legal tool source blueprint admin`，footer 写 `Refs: LMA-FB-018`。
