...
- 当前分支：`master`
- 当前阶段：门店预约 admin 员工名册配置组件拆分切片。
- 最近完成：新增 `StoreAppointmentStaffRosterPanel.vue` 和组件测试，`StoreAppointmentsPage.vue` 已改为引用该组件。
- 未完成：本地提交、生产发布。
...
- `npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentStaffRosterPanel.test.ts`
...
- [反馈编号：无] RED：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentStaffRosterPanel.test.ts` 失败于 `StoreAppointmentStaffRosterPanel.vue` 不存在。
- [反馈编号：无] GREEN：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentStaffRosterPanel.test.ts` 通过，1 个测试文件、4 项。
...
- [反馈编号：无] 收口：`npm.cmd run quality` 通过，48 个测试文件、279 项，`vue-tsc --noEmit` 和 `vite build` 通过；保留既有 PURE 注释和 chunk size warning。
...
- 本地台账：无
- 当前状态：员工名册组件拆分完整验证已通过，待本地提交。

## 注意事项

- admin 只调用 `miniapp-backend` 受控 API，不直连数据库。
- 员工名册组件只保存中性员工展示字段：`name/role/bio/avatarUrl/trustHighlights/enabled/projectCodes`。
- 员工名册不代表员工账号、权限、私联信息或真实排班。
...
- 提交信息建议：`refactor: extract store appointment staff roster panel`，正文或 footer 写 `Refs: none`。

## 下一步建议

1. 本地提交本轮员工名册组件拆分切片。
2. 下一切片可继续拆分预约规则配置 block。
