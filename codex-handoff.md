...
- 当前分支：`master`
- 当前阶段：门店预约 admin 预约详情抽屉组件拆分切片。
- 最近完成：新增 `StoreAppointmentDetailDrawer.vue` 和组件测试，`StoreAppointmentsPage.vue` 已改为引用该组件。
- 未完成：本地提交、生产发布。
...
- `npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentDetailDrawer.test.ts`
- `npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentDetailDrawer.test.ts src/pages/store-appointments/StoreAppointmentsPage.test.ts`
...
- [反馈编号：无] RED：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentDetailDrawer.test.ts` 失败于 `StoreAppointmentDetailDrawer.vue` 不存在。
- [反馈编号：无] GREEN：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentDetailDrawer.test.ts` 通过，1 个测试文件、2 项。
- [反馈编号：无] 页面回归：`npm.cmd run test -- --run src/pages/store-appointments/components/StoreAppointmentDetailDrawer.test.ts src/pages/store-appointments/StoreAppointmentsPage.test.ts` 通过，2 个测试文件、30 项。
- [反馈编号：无] 定向组合：`npm.cmd run test -- --run src/api/storeAppointments.test.ts scripts/check-admin-integration-ready.test.mjs src/pages/store-appointments/StoreAppointmentsPage.test.ts src/pages/store-appointments/components/StoreAppointmentDetailDrawer.test.ts src/pages/store-appointments/components/StoreAppointmentListPanel.test.ts src/pages/store-appointments/components/StoreAppointmentFilterPanel.test.ts src/pages/store-appointments/components/StoreAppointmentAdminConfigContractPanel.test.ts src/pages/store-appointments/components/StoreAppointmentConfigReadinessPanel.test.ts src/pages/store-appointments/components/StoreAppointmentBookingConfigSnapshotPanel.test.ts src/pages/store-appointments/components/storeAppointmentConfigPanelUtils.test.ts src/pages/store-appointments/components/StoreAppointmentConfigRollbackPanel.test.ts src/pages/store-appointments/components/StoreAppointmentStoreProfilePanel.test.ts src/pages/store-appointments/components/StoreAppointmentServiceCatalogPanel.test.ts src/pages/store-appointments/components/StoreAppointmentStaffRosterPanel.test.ts src/pages/store-appointments/components/StoreAppointmentRulesPanel.test.ts` 通过，15 个测试文件、73 项。
- [反馈编号：无] 收口：`npm.cmd run admin:check` 通过，PASS 13 / WARN 1 / FAIL 0；WARN 为本地 8080 后端未启动。
- [反馈编号：无] 收口：`npm.cmd run quality` 通过，56 个测试文件、294 项，`vue-tsc --noEmit` 和 `vite build` 通过；保留既有 PURE 注释和 chunk size warning。
...
- 本地台账：无
- 当前状态：预约详情抽屉组件拆分完整验证已通过，待本地提交。

## 注意事项

- admin 只调用 `miniapp-backend` 受控 API，不直连数据库。
- 本轮只拆分预约详情抽屉 UI 组件，不新增后端接口、不改变详情加载、状态更新和列表刷新逻辑。
- 详情抽屉组件只通过 props 接收详情、loading、状态更新中和权限状态，通过 update-status 事件回传父组件。
...
- 本轮提交信息建议：`refactor: extract store appointment detail drawer`，正文或 footer 写 `Refs: none`。

## 下一步建议

1. 本地提交本轮预约详情抽屉组件拆分切片。
2. 下一阶段继续评估门店预约页面共享状态展示工具收口或组件公共类型抽取，但不能新增禁区业务能力。
