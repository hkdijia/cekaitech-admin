...
- 当前分支：`master`
- 当前阶段：门店预约 admin 配置 API client 切片。
- 最近完成：新增四配置块 admin 配置 API client，覆盖门店资料、项目目录、员工名册、预约规则的 GET/PUT 封装，并为写请求传递 `X-Request-Id`。
- 未完成：配置编辑表单、保存错误态映射、回滚预览/确认/执行、生产发布。
...
- [反馈编号：无] 收口：`npm.cmd run quality` 通过，44 个测试文件、244 项，`vue-tsc --noEmit` 和 `vite build` 通过；保留既有 PURE 注释和 chunk size warning。
...
- 本地台账：无
- 当前状态：API client 完整质量验证已通过，待本地提交。

## 注意事项

- admin 只调用 `miniapp-backend` 受控 API，不直连数据库。
- 当前只是前端契约同步，不代表页面已经可以保存配置。
- 新增 API client 只提供前端调用封装，不代表页面已经开放配置编辑。
...
1. 完成本轮本地提交。
2. 下一切片进入配置编辑页面，先做门店资料 block 的读取、草稿、保存状态和错误保留输入。
3. 门店资料 block 绿灯后再扩展项目、员工、规则和回滚。
